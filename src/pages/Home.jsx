...
//     // Check if response.data is an array or has a content property
//     const bookingsData = Array.isArray(response.data) ? response.data : response.data.content || [];
    
//     const sortedBookings = bookingsData.sort((a, b) => new Date(b.startDate) - new Date(a.startDate));
...
//   try {
//     const response = await apiClient.get("/store/all");
//     const storesData = Array.isArray(response.data) ? response.data : response.data.content || [];
...
//   try {
//     const response = await apiClient.get("/vehicle/all");
//     const bikesData = Array.isArray(response.data) ? response.data : response.data.content || [];
//     setBikes(bikesData);
//   } catch (error) {
//     console.error("Error fetching bikes data:", error);
//   }
...
const Home = () => {
  ...
  useEffect(() => {
    ...
    const fetchAllBookings = async () => {
      try {
        // Fetch all bookings without pagination to get complete data for filtering
        const response = await apiClient.get("/booking/all", {
          params: { page: 0, size: 10000, sortBy: 'id', sortDirection: 'desc' }
        });
        ...
        const todaysBookingsFiltered = bookingWithUsernames.filter(booking => {
          const bookingDate = new Date(booking.startDate).toISOString().split('T')[0];
          return bookingDate === todayString;
        });
        setTodaysBookings(todaysBookingsFiltered);

        // Filter ongoing bookings (assuming status 'START_TRIP' means ongoing)
        const ongoing = bookingWithUsernames.filter(booking => 
          booking.status === 'START_TRIP' || booking.status === 'ONGOING'
        );
        setOngoingBookings(ongoing);
  ...
  const handleViewVerifiedUsers = () => {
    ...
    setShowTodaysBookings(false);
    setShowOngoingBookings(false);
    setDisplayedUsers(verifiedUsers);
  ...
  const handleViewUnverifiedUsers = () => {
    ...
    setShowTodaysBookings(false);
    setShowOngoingBookings(false);
    setDisplayedUsers(unverifiedUsers);
